# Documentation for <base/Clopts.hh>
...
Everything in this library is defined in the `base::cmd` namespace.
...
int main(int argc, char** argv) {
    ...
    if (ints.empty()) std::println("No ints!");
    else for (auto i : ints) std::println("{}", i);
}
```
...
// Default size is 10.
std::println("Size: {}", opts.get<"--size">(10));
```
...
static void custom_help(
    ...
    std::println(stderr, "{}{}", program_name, msg);
    std::println(stderr, "Print whatever you want here");
    std::exit(1);
}
